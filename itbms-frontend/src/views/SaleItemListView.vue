...
import {CirclePlus,Package,ChartNoAxesGantt, ArrowDownWideNarrow, ArrowUpWideNarrow, List} from 'lucide-vue-next';
...
const fetchSaleItems = async () => {
  try {
    ...
    const res = await fetchAllSaleItemsBySeller(auth, pagination.page, pagination.size, sortOption.sortField, sortOption.sortDirection)
...
const saveSessionData = () => {
  sessionStorage.setItem('sortOptionSeller', JSON.stringify({
    sortField: sortOption.sortField,
    sortDirection: sortOption.sortDirection
  }))

  sessionStorage.setItem('paginationOptionSeller', JSON.stringify({
...
watch([()=> pagination.size, sortOption], () => {
  pagination.page = 0
  saveSessionData()
  fetchSaleItems()
},{ deep: true })
