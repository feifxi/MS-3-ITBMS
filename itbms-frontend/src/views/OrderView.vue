...
<template>
  <div class="px-4 sm:px-16 py-8 bg-gradient-to-br from-purple-50 via-pink-50 to-white min-h-screen">
    <div class="bg-white px-4 sm:px-16 py-8 rounded-2xl shadow-2xl max-w-7xl mx-auto border border-purple-100">
      <!-- Header -->
      <div class="mb-8 pb-6 border-b-2 border-purple-200">
        <div class="flex items-center justify-center gap-3 mb-3">
          <div class="bg-gradient-to-r from-purple-500 to-pink-500 rounded-full p-3">
      ...
      <div v-else-if="!orders || orders.length === 0" class="text-center py-16">
        <div class="bg-gradient-to-r from-purple-100 to-pink-100 rounded-full w-32 h-32 flex items-center justify-center mx-auto mb-6">
      ...
      <div v-else class="space-y-4">
        ...
        >
          <div class="flex flex-col sm:flex-row items-start gap-4 p-6 bg-gradient-to-r from-purple-50/50 to-pink-50/50">
            <!-- Product Image -->
            <div class="flex-shrink-0 mx-auto sm:mx-0">
              <img
                v-if="order.orderItems.length > 0 && order.orderItems[0].saleItemImages && order.orderItems[0].saleItemImages.length > 0"
              ...
              />
              <div v-else class="w-20 h-20 sm:w-24 sm:h-24 bg-gradient-to-br from-purple-100 to-pink-100 rounded-xl flex items-center justify-center border-2 border-purple-200 shadow-md">
            ...
            <!-- Order Info -->
            <div class="flex-1 min-w-0 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 w-full">
              <div class="flex-1 min-w-0 text-center sm:text-left">
                <div class="flex items-center justify-center sm:justify-start gap-3 mb-2 flex-wrap">
                  ...
                  <span 
                    ...
                    :class="{
                      'bg-yellow-100 text-yellow-800 border-yellow-300': order.orderStatus === 'PENDING',
                      'bg-green-100 text-green-800 border-green-300': order.orderStatus === 'COMPLETED',
                      'bg-red-100 text-red-800 border-red-300': order.orderStatus === 'CANCELLED',
                      'bg-blue-100 text-blue-800 border-blue-300': order.orderStatus === 'PROCESSING',
                      'bg-gray-100 text-gray-800 border-gray-300': !['PENDING', 'COMPLETED', 'CANCELLED', 'PROCESSING'].includes(order.orderStatus)
                    }"
                  >
                    {{ order.orderStatus }}
                  </span>
              ...
              <!-- Price -->
              <div class="flex-shrink-0 text-center sm:text-right bg-white rounded-xl p-4 shadow-md border-2 border-purple-100">
